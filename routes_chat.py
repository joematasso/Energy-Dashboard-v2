...
def get_messages(conv_id):
    ...
    rows = db.execute("""
        SELECT m.*, t.display_name, t.photo_url, tm.name as team_name, tm.color as team_color
        FROM messages m LEFT JOIN traders t ON m.sender=t.trader_name
        LEFT JOIN teams tm ON t.team_id=tm.id
...
def send_message(conv_id):
    ...
    if not member:
        return jsonify({'success': False, 'error': 'Not a member'}), 403
    # Block sending to system (broadcast-only) conversations
    conv = db.execute("SELECT type FROM conversations WHERE id=?", (conv_id,)).fetchone()
    if conv and conv['type'] in ('system', 'admin_inbox'):
...
# ---------------------------------------------------------------------------
# Message Admin (from Help panel)
# ---------------------------------------------------------------------------
@chat_bp.route('/api/chat/message-admin', methods=['POST'])
def message_admin():
    db = get_db()
    data = request.get_json()
    ...
    if not sender or not body:
        return jsonify({'success': False, 'error': 'Sender and message required'}), 400
    # Verify sender exists
    trader = db.execute("SELECT trader_name, display_name FROM traders WHERE trader_name=?", (sender,)).fetchone()
    if not trader:
        return jsonify({'success': False, 'error': 'Invalid trader'}), 403
    # Find or create "Admin Messages" conversation (type='admin_inbox')
    convo = db.execute("SELECT id FROM conversations WHERE name='Admin Messages' AND type='admin_inbox'").fetchone()
    if not convo:
        cur = db.execute("INSERT INTO conversations (name, type) VALUES ('Admin Messages', 'admin_inbox')")
    ...
    existing = db.execute("SELECT conversation_id FROM conversation_members WHERE conversation_id=? AND trader_name=?",
                          (convo_id, sender)).fetchone()
    if not existing:
        db.execute("INSERT INTO conversation_members (conversation_id, trader_name) VALUES (?, ?)", (convo_id, sender))
    # Build message text
    display = trader['display_name'] or sender
    msg_text = f"[From {display}] "
    if subject:
        msg_text += f"{subject}: "
    msg_text += body
    db.execute("INSERT INTO messages (conversation_id, sender, text) VALUES (?, ?, ?)", (convo_id, sender, msg_text))
    db.commit()
    # Also store as an admin broadcast record so admin panel can see it
    try:
        db.execute("INSERT INTO admin_broadcasts (subject, body, priority) VALUES (?, ?, 'normal')",
                   (f"Support from {display}: {subject or '(no subject)'}", body))
        db.commit()
    except Exception:
        pass
    return jsonify({'success': True})
