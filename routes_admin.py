...
def send_broadcast():
    ...
    try:
        ...
        for t in traders:
            existing = db.execute("SELECT id FROM conversation_members WHERE conversation_id=? AND trader_name=?",
                                  (sys_convo_id, t['trader_name'])).fetchone()
            if not existing:
                db.execute("INSERT INTO conversation_members (conversation_id, trader_name) VALUES (?, ?)",
