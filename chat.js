...
function renderConvoList() {
  ...
  list.innerHTML = CHAT_STATE.conversations.map(c => {
    ...
    if(c.type === 'system') name = '📡 ' + (c.name || 'System');
    const icon = c.type === 'team' ? '🏢' : c.type === 'group' ? '👥' : c.type === 'system' ? '📡' : '';
...
async function openConvo(convId) {
  ...
  if(convo.type==='system') name = '📡 ' + (convo.name || 'System Broadcasts');
  document.getElementById('chatTitle').textContent = name;

  // Hide input for system (read-only) conversations
  const chatInputWrap = document.getElementById('chatInputWrap');
  if(chatInputWrap) chatInputWrap.style.display = convo.type === 'system' ? 'none' : '';

  // Header avatar
  const headerAvatar = document.getElementById('chatHeaderAvatar');
  const isGroupOrTeam = convo.type === 'group' || convo.type === 'team';
  if (convo.type === 'system') {
  ...
  if(convo.type !== 'system') document.getElementById('chatInput').focus();
}
...
async function chatSend() {
  ...
  if(CHAT_STATE.activeConvo.type === 'system') { toast('This is a read-only broadcast channel','error'); return; }
  const input = document.getElementById('chatInput');
