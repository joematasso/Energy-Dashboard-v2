...
function renderMessages(msgs) {
  ...
  container.innerHTML = msgs.map(m => {
    ...
    return `<div class="chat-msg ${isMe?'me':'other'}">
      ${pinIndicator}
      ${!isMe?`<div class="msg-sender">${!isMe && m.photo_url ? `<img src="${m.photo_url}" style="width:20px;height:20px;border-radius:50%;object-fit:cover;vertical-align:middle;margin-right:4px">` : ''}${teamDot}${m.display_name}</div>`:''}
  ...
  list.innerHTML = `<div style="padding:8px"><h4 style="margin:0 0 8px;font-size:13px;color:var(--text-dim)">Start a conversation</h4>
    ...
    <div id="chatTraderPicker">${traders.map(t=>{
      ...
      const avatar = t.photo_url
        ? `<div class="convo-avatar"><img src="${t.photo_url}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"></div>`
        : `<div class="convo-avatar">${(t.display_name||'?')[0].toUpperCase()}</div>`;
      return `<div class="chat-convo-item" onclick="startDm('${t.trader_name}')">${avatar}<div class="convo-info"><div class="convo-name">${teamDot} ${t.display_name}</div><div class="convo-preview">${t.firm||''}</div></div></div>`;
    }).join('')}</div>
  ...
  try {
    ...
    if(sock) {
      ...
      sock.on('admin_broadcast', function(data) {
        ...
        // Reload conversations to pick up the admin/system conversation
        loadConversations();
        updateUnreadBadge();
