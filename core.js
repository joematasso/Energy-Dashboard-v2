...
const STATE = {
  ...
  alerts: JSON.parse(localStorage.getItem('ng_alerts') || '[]'),
  notifications: JSON.parse(localStorage.getItem('ng_notifications') || '[]'),
  pendingOrders: JSON.parse(localStorage.getItem('ng_pending_orders') || '[]'),
...
function renderNewsTicker() {
  ...
  const buildHTML = (items) => items.map(item => {
    ...
    const safeHL = item.headline.replace(/'/g, "\\'");
    const safeDesc = (item.description || '').replace(/'/g, "\\'");
    const url = item.url || '';
    return `<span class="news-tick" onclick="openNewsModal('${safeHL}','${safeDesc}','${url}')"><span class="news-tick-tag ${tag.cls}">${tag.label}</span>${item.headline}<span class="news-tick-src">${item.time}</span></span><span class="news-tick-sep">◆</span>`;
  }).join('');

  // Duplicate content for seamless looping
  const segment = buildHTML(interleaved);
  inner.innerHTML = segment + segment;
