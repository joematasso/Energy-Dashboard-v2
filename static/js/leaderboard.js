...
function renderLeaderboardData(serverData, isLive) {
  ...
  if (lbTab === 'rankings') {
    ...
    else{tb.innerHTML=tl.map((t,i)=>{const ar=t.members.reduce((s,m)=>s+m.ret,0)/t.members.length;const best=t.members.sort((a,b)=>b.ret-a.ret)[0];return `<tr class="blotter-row"><td style="font-weight:700">${i+1}</td><td><span style="display:inline-flex;align-items:center;gap:6px"><span style="width:10px;height:10px;border-radius:50%;background:${t.color}"></span>${t.name}</span></td><td>${t.members.length}</td><td><span class="perf-badge ${t.totalPnl>=0?'up':'down'}">${t.totalPnl>=0?'+':'-'}$${Math.abs(t.totalPnl).toLocaleString(undefined,{maximumFractionDigits:0})}</span></td><td><span class="perf-badge ${ar>=0?'up':'down'}">${ar>=0?'+':''}${ar.toFixed(1)}%</span></td><td>${best?best.name:'\u2014'}</td></tr>`;}).join('');}
    // Draw team bar chart
